...
- [x] 1. Extend type definitions and shared utilities
  - [x] 1.1 Add Phase 2 types to `src/types.ts`
  ...
  - [x] 1.2 Create shared `splitSentences()` utility in `src/utils.ts`
...
- [x] 2. Implement ConsentRecord and session consent management
  - [x] 2.1 Add consent management methods to `src/session-manager.ts`
  ...
  - [x] 2.2 Write property test for consent immutability
    - **Property 2: Consent Record Immutability**
    - **Validates: Requirements 2.4**

  - [x] 2.3 Write property test for session data purge completeness
...
- [x] 4. Implement ToneChecker
  - [x] 4.1 Create `src/tone-checker.ts` with pattern-based detection
  ...
  - [x] 4.2 Write property test for tone checker detection completeness
    - **Property 5: Tone Checker Detection Completeness**
    - **Validates: Requirements 3.2, 3.3, 3.4, 3.5, 3.6**

  - [x] 4.3 Write property test for tone violation stripping correctness
    - **Property 6: Tone Violation Stripping Correctness**
    - **Validates: Requirements 3.8**

  - [x] 4.4 Write property test for scope acknowledgment conditional append
    - **Property 7: Scope Acknowledgment Conditional Append**
    - **Validates: Requirements 3.10, 6.7**

  - [x] 4.5 Write property test for marker elimination
    - **Property 20: Marker Elimination After Tone Check**
    - **Validates: Requirements 11.2, 11.5**

- [x] 5. Enhance MetricsExtractor with pause classification, energy variation, and filler classification
  - [x] 5.1 Add pause classification to `src/metrics-extractor.ts`
  ...
  - [x] 5.2 Write property test for pause classification correctness
    - **Property 9: Pause Classification Correctness**
    - **Validates: Requirements 5.1, 5.2, 5.3**

  - [x] 5.3 Add speech energy variation computation to `src/metrics-extractor.ts`
  ...
  - [x] 5.4 Write property test for energy profile computation correctness
    - **Property 10: Energy Profile Computation Correctness**
    - **Validates: Requirements 5.5, 5.7, 5.8**

  - [x] 5.5 Write property test for energy gain invariance
    - **Property 11: Energy Gain Invariance**
    - **Validates: Requirements 5.6**

  - [x] 5.6 Enhance filler word classification in `src/metrics-extractor.ts`
  ...
  - [x] 5.7 Write property test for filler word classification consistency
...
- [x] 7. Enhance TTSEngine with structured trimming and configurable time limits
  - [x] 7.1 Update `src/tts-engine.ts` with safety margin and structured trimming
  ...
  - [x] 7.2 Write property test for duration estimation with safety margin
    - **Property 13: Duration Estimation with Safety Margin**
    - **Validates: Requirements 6.2**

  - [x] 7.3 Write property test for structured trimming correctness
    - **Property 14: Structured Trimming Correctness**
    - **Validates: Requirements 6.3, 6.4, 6.5, 6.6**

- [x] 8. Enhance EvaluationGenerator with structure commentary, markers, and pass-rate reporting
  - [x] 8.1 Update LLM prompt in `src/evaluation-generator.ts` for structure commentary
  ...
  - [x] 8.2 Update `renderScript()` in `src/evaluation-generator.ts` with marker emission
  ...
  - [x] 8.3 Write property test for null structure commentary omission
    - **Property 8: Null Structure Commentary Omission**
    - **Validates: Requirements 4.3**

  - [x] 8.4 Add pass-rate reporting and short-form fallback to `src/evaluation-generator.ts`
  ...
  - [x] 8.5 Write property test for evidence pass rate computation
    - **Property 18: Evidence Pass Rate Computation**
    - **Validates: Requirements 1.6**

  - [x] 8.6 Write property test for extended structural shape invariant
    - **Property 1: Extended Structural Shape Invariant**
    - **Validates: Requirements 1.1, 4.9, 5.10**

  - [x] 8.7 Write property test for short-form fallback shape and evidence
...
- [x] 10. Implement name redaction with public evaluation types
  - [x] 10.1 Update redaction in `src/evaluation-generator.ts`
  ...
  - [x] 10.2 Write property test for redaction correctness
    - **Property 15: Redaction Correctness**
    - **Validates: Requirements 8.1, 8.2, 8.4, 8.5**

  - [x] 10.3 Write property test for public output redaction completeness
    - **Property 21: Public Output Redaction Completeness**
    - **Validates: Requirements 8.1, 8.4, 8.5**

- [x] 11. Wire the full Phase 2 pipeline in SessionManager
  - [x] 11.1 Update `generateEvaluation()` in `src/session-manager.ts` to orchestrate the Phase 2 pipeline
  ...
  - [x] 11.2 Add consent WebSocket handlers to `src/server.ts`
  ...
  - [x] 11.3 Update `saveOutputs()` in `src/session-manager.ts` and `src/file-persistence.ts`
  ...
  - [x] 11.4 Write property test for consent round-trip in saved outputs
    - **Property 3: Consent Round-Trip in Saved Outputs**
    - **Validates: Requirements 2.6**

- [x] 12. Update Web UI for consent, time limit, and duration display
  - [x] 12.1 Update `public/index.html` with consent form and time limit controls
...
- [x] 13. Implement quality warning propagation and consistency monitoring
  - [x] 13.1 Update quality warning logic in `src/session-manager.ts` and `src/evaluation-generator.ts`
  ...
  - [x] 13.2 Write property test for quality warning threshold correctness
    - **Property 17: Quality Warning Threshold Correctness**
    - **Validates: Requirements 10.1**

  - [x] 13.3 Add consistency monitoring telemetry (background, non-blocking)
  ...
  - [x] 13.4 Write property test for cosine similarity computation
