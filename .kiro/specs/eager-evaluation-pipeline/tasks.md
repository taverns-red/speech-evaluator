...
- [x] 1. Add types and session field extensions
  - [x] 1.1 Add `EagerStatus`, `PipelineStage`, `EvaluationCache`, and `Deferred<T>` types to `src/types.ts`; add `createDeferred<T>()` factory to `src/utils/deferred.ts`
  ...
  - [x] 1.2 Add `eagerStatus`, `eagerRunId`, `eagerPromise`, and `evaluationCache` fields to `Session` interface in `src/types.ts`
    ...
    - _Requirements: 2.1_
  - [x] 1.3 Add `pipeline_progress` variant to `ServerMessage` union type in `src/types.ts`
    - Add `{ type: "pipeline_progress"; stage: PipelineStage; runId: number; message?: string }`
    - _Requirements: 9.1_
  - [x] 1.4 Initialize new session fields in `SessionManager.createSession()` in `src/session-manager.ts`
    - Set `eagerStatus: "idle"`, `eagerRunId: null`, `eagerPromise: null`, `evaluationCache: null` in the session object literal
    - _Requirements: 2.1_

- [x] 2. Implement SessionManager cache validity, eager pipeline core, and centralized cleanup
  - [x] 2.1 Add `isEagerCacheValid()` method to `SessionManager` in `src/session-manager.ts`
  ...
  - [x] 2.2 Write property test for cache validity invariant in `src/session-manager.property.test.ts`
    - **Property 4: Cache validity invariant**
    - Include `evaluationPublic !== null` in the validity assertion
    - **Validates: Requirements 6.1**
  - [x] 2.3 Add `clearEagerState()` and `cancelEagerGeneration()` methods to `SessionManager` in `src/session-manager.ts`
  ...
  - [x] 2.4 Implement `runEagerPipeline()` method on `SessionManager` in `src/session-manager.ts`
  ...
  - [x] 2.5 Write property test for state invariants during eager execution in `src/session-manager.property.test.ts`
  ...
  - [x] 2.6 Write property test for eager status transition sequence in `src/session-manager.property.test.ts`
  ...
  - [x] 2.7 Write property test for failure handling in `src/session-manager.property.test.ts`
  ...
  - [x] 2.8 Write property test for RunId staleness in `src/session-manager.property.test.ts`
  ...
  - [x] 2.9 Write property test for single-flight per RunId in `src/session-manager.property.test.ts`
  ...
  - [x] 2.10 Write property test for state precondition guard in `src/session-manager.property.test.ts`
...
- [x] 3. Implement cache invalidation and lifecycle methods
  - [x] 3.1 Add `invalidateEagerCache()` method to `SessionManager` in `src/session-manager.ts`
    - Call `this.cancelEagerGeneration(sessionId)` — increments `runId` and clears all eager fields
    - _Requirements: 6.2_
  - [x] 3.2 Modify `startRecording()` in `src/session-manager.ts` to reset eager state
  ...
  - [x] 3.3 Modify `panicMute()` in `src/session-manager.ts` to clear eager state and cache
  ...
  - [x] 3.4 Modify `revokeConsent()` in `src/session-manager.ts` to clear eager state and cache
  ...
  - [x] 3.5 Modify `purgeSessionData()` to clear eager state and cache via SessionManager
    ...
    - _Requirements: 6.7_
  - [x] 3.6 Write property tests for cache invalidation and lifecycle in `src/session-manager.property.test.ts`
...
- [x] 4. Checkpoint - Ensure all SessionManager tests pass
  - Run `vitest --run` for session-manager test files
  - Ensure all tests pass, ask the user if questions arise

- [x] 5. Modify server delivery and stop-recording handlers
  - [x] 5.1 Modify `handleStopRecording()` in `src/server.ts` to kick off eager pipeline
  ...
  - [x] 5.2 Rewrite `handleDeliverEvaluation()` in `src/server.ts` with three-branch delivery logic
  ...
  - [x] 5.3 Modify `handleSetTimeLimit()` in `src/server.ts` to invalidate eager cache and send UI reset
    ...
    - _Requirements: 6.2_
  - [x] 5.4 Write property test for cache-hit delivery skips generation in `src/server.property.test.ts`
  ...
  - [x] 5.5 Write property test for fallback delivery in `src/server.property.test.ts`
  ...
  - [x] 5.6 Write property test for behavioral boundary at server layer in `src/server.property.test.ts`
  ...
  - [x] 5.7 Write unit tests for delivery and invalidation edge cases in `src/server.test.ts`
...
- [x] 7. Update frontend UI for eager pipeline
  - [x] 7.1 Add `pipeline_progress` message handler and client state in `public/index.html`
  ...
  - [x] 7.2 Implement `updateProcessingIndicator()` function with stage-specific text in `public/index.html`
  ...
  - [x] 7.3 Update Deliver button gating logic in PROCESSING state in `public/index.html`
  ...
  - [x] 7.4 Reset pipeline client state on new recording and session transitions in `public/index.html`
