...
- [x] 1. Add deferral state variables and helper functions
  - [x] 1.1 Add `playbackInstanceToken`, `deferredIdleTransition`, `pendingIdleFromServer` variables and `applyDeferredIdle(token)`, `cancelDeferredIdle()`, `transitionToIdle()` functions to the client script in `public/index.html`
...
- [x] 2. Modify handleStateChange to defer IDLE during active playback
  - [x] 2.1 Update `handleStateChange(newState)` in `public/index.html` to check `ttsPlaying` before applying IDLE transition
  ...
  - [x] 2.2 Write property test: Deferred IDLE round-trip (Property 1)
    - **Property 1: Deferred IDLE round-trip**
    - **Validates: Requirements 1.1, 1.2, 1.4, 3.2**

  - [x] 2.3 Write property test: Immediate IDLE when not playing (Property 2)
    - **Property 2: Immediate IDLE when not playing**
    - **Validates: Requirements 1.3**

  - [x] 2.4 Write property test: Duplicate idle idempotency (Property 3)
    - **Property 3: Duplicate idle idempotency**
    - **Validates: Requirements 1.6**

  - [x] 2.5 Write property test: Stale token rejection (Property 4)
    - **Property 4: Stale token rejection**
    - **Validates: Requirements 1.7**

- [x] 3. Modify handleTTSAudio to bind playback instance token
  - [x] 3.1 Update `handleTTSAudio(audioData)` in `public/index.html` to increment `playbackInstanceToken` and capture it in the `onended`/`onerror` closures
...
- [x] 4. Modify panic mute, revoke consent, and WebSocket close to cancel deferral
  - [x] 4.1 Update `onPanicMute()` in `public/index.html` to call `forceStopTtsAndCancelDeferral()` instead of separate `cancelDeferredIdle()` + `stopTTSPlayback()`
    - Single call handles: bump token, clear deferral, stop audio, clear ttsPlaying
    - _Requirements: 2.1, 2.2, 2.3_

  - [x] 4.2 Update `onRevokeConsent()` in `public/index.html` to call `forceStopTtsAndCancelDeferral()` instead of separate `cancelDeferredIdle()` + `stopTTSPlayback()`
    - Same single-call pattern as panic mute
    - _Requirements: 2.1, 2.3_

  - [x] 4.3 Update `ws.onclose` handler in `public/index.html` to fail-safe on playback, deferral, latch, OR non-IDLE state
  ...
  - [x] 4.4 Write property test: Panic mute always clears deferral and playback (Property 5)
    - **Property 5: Panic mute always clears deferral and playback**
    - **Validates: Requirements 2.1, 2.2**

  - [x] 4.5 Write property test: Abort during deferral triggers fail-safe (Property 6)
    - **Property 6: Abort during deferral triggers fail-safe**
    - **Validates: Requirements 4.1, 4.2, 4.3**

  - [x] 4.6 Write property test: Cancellation invalidates token (Property 7)
    - **Property 7: Cancellation invalidates token**
    - **Validates: Requirements 1.7, 2.3**

  - [x] 4.7 Write property test: play() rejection doesn't deadlock (Property 8)
    - **Property 8: play() rejection doesn't deadlock**
    - **Validates: Requirements 4.3**

  - [x] 4.8 Write property test: No dead deferral (Property 9)
  ...
  - [x] 4.9 Write property test: No premature replay visibility (Property 10)
  ...
  - [x] 4.10 Write property test: pendingIdleFromServer latch correctness (Property 11)
...
- [x] 5. Checkpoint — Verify all paths
...
- [x] 6. Write unit tests for specific scenarios
  - [x] 6.1 Write unit tests for deferral edge cases
